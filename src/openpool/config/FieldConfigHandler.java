...
public class FieldConfigHandler extends ConfigHandlerAbstractImpl {
    ...
    public void draw() {
        ...
        if (op.pa.mousePressed) {
            ...
            for (int i = 0; i < 2; i++) {
                int distanceSq = getDistanceSq(mx, my,
                        op.getCombinedImageCorner(i).x,
                        op.getCombinedImageCorner(i).y);
                if (distanceSq < mSq) {
                    mSq = distanceSq;
                    selected = i;
                }
            ...
            onArea = isInArea(mx, my, op.getCombinedImageCorner(0).x,
                    op.getCombinedImageCorner(0).y,
                    op.getCombinedImageCorner(1).x,
                    op.getCombinedImageCorner(1).y);
    ...
    private boolean isInArea(int x, int y, int Area_top_left_x,
            int Area_top_left_y, int Area_bottom_right_x,
            int Area_bottom_right_y) {
