...
public class CameraPositionConfigHandler extends ConfigHandlerAbstractImpl {
    ...
    public void draw() {
        ...
        cam1tl_ScreenAxis = ImagetoScreen(cam1tl_imageAxis, tl, br, depthWidth,
                depthHeight, 0, ballDetector.getCam1Width());
        cam1br_ScreenAxis = ImagetoScreen(cam1br_imageAxis, tl, br, depthWidth,
                depthHeight, 0, ballDetector.getCam1Width());
        ...
        if (camCount >= 2) {
            ...
            cam2br_imageaxis.x = cam2tl_imageaxis.x
                    + ballDetector.getCam2Width();
            cam2br_imageaxis.y = cam2tl_imageaxis.y
                    + ballDetector.getCam2Height();

            cam2tl_ScreenAxis = ImagetoScreen(cam2tl_imageaxis, tl, br,
                    depthWidth, depthHeight, 1, ballDetector.getCam1Width());
            cam2br_ScreenAxis = ImagetoScreen(cam2br_imageaxis, tl, br,
                    depthWidth, depthHeight, 1, ballDetector.getCam1Width());
        }
        ...
        if (op.pa.mousePressed && selectedCam >= 0) {
            ballDetector.setCamImageCorner(
                    selectedCam,
                    ScreentoImage(mp, tl, br, depthWidth, depthHeight,
                            selectedCam, ballDetector.getCam1Width()));
        } else {
            selectedCam = -1;
            ...
            for (int i = 0; i < camCount; i++) {
                ...
                Point screenPt = ImagetoScreen(
                        ballDetector.getCamImageCorner(i), tl, br, depthWidth,
                        depthHeight, i, ballDetector.getCam1Width());

                int distanceSq = getDistanceSq(mp.x, mp.y, screenPt.x,
                        screenPt.y);
        ...
        op.pa.line(cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.y,
                cam1br_ScreenAxis.x, cam1tl_ScreenAxis.y);
        op.pa.line(cam1br_ScreenAxis.x, cam1tl_ScreenAxis.y,
                cam1br_ScreenAxis.x, cam1br_ScreenAxis.y);
        op.pa.line(cam1br_ScreenAxis.x, cam1br_ScreenAxis.y,
                cam1tl_ScreenAxis.x, cam1br_ScreenAxis.y);
        op.pa.line(cam1tl_ScreenAxis.x, cam1br_ScreenAxis.y,
                cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.y);

        // draw an arrow
        op.pa.line(cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.y,
                cam1tl_ScreenAxis.x + 8, cam1tl_ScreenAxis.y + 4);
        op.pa.line(cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.y,
                cam1tl_ScreenAxis.x + 4, cam1tl_ScreenAxis.y + 8);
        op.pa.line(cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.y,
                cam1tl_ScreenAxis.x + 10, cam1tl_ScreenAxis.y + 10);

        // draw xy
        op.pa.text("X:", cam1tl_ScreenAxis.x + 20, cam1tl_ScreenAxis.y + 20);
        op.pa.text(cam1tl_ScreenAxis.x, cam1tl_ScreenAxis.x + 30,
                cam1tl_ScreenAxis.y + 20);
        op.pa.text("Y:", cam1tl_ScreenAxis.x + 20, cam1tl_ScreenAxis.y + 34);
        op.pa.text(cam1tl_ScreenAxis.y, cam1tl_ScreenAxis.x + 30,
                cam1tl_ScreenAxis.y + 34);

        if (camCount >= 2) {
            // draw cam2 image bounding
            op.pa.stroke(0, 255, 0);
            op.pa.line(cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.y,
                    cam2br_ScreenAxis.x, cam2tl_ScreenAxis.y);
            op.pa.line(cam2br_ScreenAxis.x, cam2tl_ScreenAxis.y,
                    cam2br_ScreenAxis.x, cam2br_ScreenAxis.y);
            op.pa.line(cam2br_ScreenAxis.x, cam2br_ScreenAxis.y,
                    cam2tl_ScreenAxis.x, cam2br_ScreenAxis.y);
            op.pa.line(cam2tl_ScreenAxis.x, cam2br_ScreenAxis.y,
                    cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.y);

            // draw an arrow
            op.pa.line(cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.y,
                    cam2tl_ScreenAxis.x + 8, cam2tl_ScreenAxis.y + 4);
            op.pa.line(cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.y,
                    cam2tl_ScreenAxis.x + 4, cam2tl_ScreenAxis.y + 8);
            op.pa.line(cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.y,
                    cam2tl_ScreenAxis.x + 10, cam2tl_ScreenAxis.y + 10);

            // draw xy
            op.pa.text("X:", cam2tl_ScreenAxis.x + 20, cam2tl_ScreenAxis.y + 20);
            op.pa.text(cam2tl_ScreenAxis.x, cam2tl_ScreenAxis.x + 30,
                    cam2tl_ScreenAxis.y + 20);
            op.pa.text("Y:", cam2tl_ScreenAxis.x + 20, cam2tl_ScreenAxis.y + 34);
            op.pa.text(cam2tl_ScreenAxis.y, cam2tl_ScreenAxis.x + 30,
                    cam2tl_ScreenAxis.y + 34);
        }
        op.pa.noStroke();
    }
     ...
     */
    private Point ImagetoScreen(Point imagePt, Point screen_tl,
            Point screen_br, int combinedDepthImageWidth,
            int combinedDepthImageHeight, int camId, int cam1Width) {

        Point retPt = new Point();
        if (camId == 0) {
            retPt.x = (int) (screen_tl.x + imagePt.x
                    * (double) (screen_br.x - screen_tl.x)
                    / (double) combinedDepthImageWidth);
            retPt.y = (int) (screen_tl.y + imagePt.y
                    * (double) (screen_br.y - screen_tl.y)
                    / (double) combinedDepthImageHeight);
        } else {
            retPt.x = (int) (screen_tl.x + (imagePt.x + cam1Width)
                    * (double) (screen_br.x - screen_tl.x)
                    / (double) combinedDepthImageWidth);
            retPt.y = (int) (screen_tl.y + imagePt.y
                    * (double) (screen_br.y - screen_tl.y)
     ...
     *            width of cam1 in image xy
     */
    private Point ScreentoImage(Point screenPt, Point screen_tl,
            Point screen_br, int combinedDepthImageWidth,
            int combinedDepthImageHeight, int camId, int cam1Width) {
        Point retPt = new Point();
        if (camId == 0) {
