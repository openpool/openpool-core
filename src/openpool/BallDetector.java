...
public class BallDetector implements Runnable {
    ...
    BallDetector(BallSystem ballSystem, SimpleOpenNI cam1, SimpleOpenNI cam2,
        ...
        currentImage = IplImage.create(cvSize(depthWidth, depthHeight),
                IPL_DEPTH_8U, 1);

        temporaryImage = IplImage.create(cvSize(depthWidth, depthHeight),
                IPL_DEPTH_8U, 4);

        diffImage = new BufferedImage(currentImage.width(),
                currentImage.height(), BufferedImage.TYPE_INT_ARGB);

        resultImage = new BufferedImage(currentImage.width(),
                currentImage.height(), BufferedImage.TYPE_INT_ARGB);
    ...
    public synchronized void run() {
        ...
        // Comment out the line below when calculating difference between full
        // color (+ alpha) images.
        ...
        // Binarization
        cvThreshold(currentImage, currentImage, getThreshold(), 255,
                CV_THRESH_BINARY);
        // cvAdaptiveThreshold(currentImage, currentImage, 255,
        ...
        int count = cvFindContours(currentImage, mem, contours,
                sizeof(CvContour.class), CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE,
                cvPoint(0, 0));
        ...
        if (count > 0) {
            ...
            for (ptr = contours; ptr != null; ptr = ptr.h_next()) {
                ...
                if (area > 4) {
                    ...
                    cvDrawContours(temporaryImage, ptr, color, CV_RGB(0, 0, 0),
    ...
    public PImage getDiffImage() {
        if (diffPImage == null) {
            ...
            DataBufferInt dbi = new DataBufferInt(diffPImage.pixels,
                    diffPImage.pixels.length);
    ...
    public PImage getImage() {
        if (resultPImage == null) {
            ...
            DataBufferInt dbi = new DataBufferInt(resultPImage.pixels,
                    resultPImage.pixels.length);
     ...
     * 
     * @param target
     *            Target IplImage object
     * @param source
     *            Source PImage object
     * @param x
     *            Target x-coordinate
     * @param y
     *            Target y-coordinate
    ...
    private void copyImage(IplImage target, PImage source, int x, int y,
        ...
        IplImage sourceImage = IplImage.createFrom((BufferedImage) source
                .getImage());
     ...
     * 
     * @param target
     *            Target IplImage (1 channel grayscale)
     * @param depthMap
     *            Source depth map
     * @param x
     *            Target x-coordinate
     ...
     *            Target height
     */
    private void fillDepthErrorHoles(IplImage target, int[] depthMap, int x,
